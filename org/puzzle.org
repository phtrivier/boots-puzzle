...
* Game UI
** TODO Add quotes between levels (this will have something displayed when
a level is cleared..)
*** TODO Add a quote for each level (a quote has a content, and an author)
*** TODO Show the quote in a nice font between levels (call this a quote mode, the only
drawing is to draw the font, and that's it
