...
* Game Engine
** TODO Make it possible to display the version and help from the wrapper script
(Reuse code in Rakefile if possible)
** TODO Console ?
* Game UI
** TODO [#A] Fix the ugly bitmap font (use a ttf if possible ... check if there is one that come for "free")
** TODO [#A] Find a icon for the application (the bare foots ?)
